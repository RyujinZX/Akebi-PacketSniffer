...
#include <sniffer/pipe/messages/PipeModifyData.h>
#include <sniffer/pipe/messages/PipePacketData.h>
...
namespace sniffer::pipe
{
	...
	namespace internal
	{
		...
		template<> struct message_to_id<messages::PipePacketData> : std::integral_constant<uint32_t, static_cast<uint32_t>(MessageIDs::PACKET_DATA)> {};
		template<> struct message_to_id<messages::PipeModifyData> : std::integral_constant<uint32_t, static_cast<uint32_t>(MessageIDs::MODIFY_DATA)> {};
	}
	...
	public:
		...
		{
			static_assert(std::is_base_of<messages::PipeMessage, T>::value, "Should be derived of MessageBase.");
		...
		{
			static_assert(std::is_base_of<messages::PipeMessage, TMessage>::value, "Should be derived of MessageBase.");
		...
		void Send(messages::PipeMessage& data);

		messages::PipeMessage* ReceiveMessage();
	...
	private:
		using UniqueCallbackFunction = void(*)(const messages::PipeMessage& pipeMessage);
